...
export default function ProfileModal({ isOpen, onClose, onNotify }) {
  ...
  return (
    <ModalForm
      isOpen={isOpen}
      onClose={isUpdating || isDeleting ? undefined : onClose}
      title="Můj profil"
      closeLabel="Zavřít"
      isDismissDisabled={isUpdating || isDeleting}
    >
      <section
        style={{
          display: "flex",
      ...
      >
        {infoItems.map((item) => (
          <div key={item.label} style={{ display: "flex", justifyContent: "space-between", gap: "0.5rem" }}>
            <span style={{ ...styles.textMuted, minWidth: "8rem" }}>{item.label}</span>
      ...
      >
        <h3 style={{ margin: 0 }}>Export &amp; Import dat</h3>
