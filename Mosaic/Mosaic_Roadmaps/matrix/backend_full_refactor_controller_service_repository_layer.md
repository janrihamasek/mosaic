...
- Move all SQL from controllers/services into repository methods.
     chceš jediné přímé zadání pro Codex: sjednotit všechny vytvořené větve do větve `architecture`, vyřešit konflikty automaticky podle jednoduchých pravidel, pushnout výsledek a smazat remote feature větve. Níže máš jeden jediný, přesný prompt, který vložíš do Codex agenta ve VS Code (spustí lokálně).
